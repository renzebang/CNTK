...
struct StreamInformation {
  StreamInformation() : m_id(0), m_storageFormat(StorageFormat::Dense), m_elementType(DataType::Unknown), m_sampleLayout(NDShape::Unknown()) {}
...
class TrainingParameterSchedule : public IDictionarySerializable {
 public:
  ...
  CNTK_API bool operator==(const TrainingParameterSchedule<T>& right) {
    return this->m_schedule == right.m_schedule && this->m_epochSize == right.m_epochSize && this->m_minibatchSize == right.m_minibatchSize;
  }
...
class DistributedLearner : public Learner {
 ...
 protected:
  ...
  // Disallow copy and move construction and assignment
  DistributedLearner(const DistributedLearner&) = delete;
  DistributedLearner& operator=(const DistributedLearner&) = delete;
  DistributedLearner& operator=(DistributedLearner&&) = delete;
  DistributedLearner(DistributedLearner&&) = delete;
};
...
namespace std {
template <>
struct hash<::CNTK::StreamInformation> {
...
class MinibatchSource : public std::enable_shared_from_this<MinibatchSource> {
 ...
 public:
  ...
  // Disallow copy and move construction and assignment
  MinibatchSource(const MinibatchSource&) = delete;
  MinibatchSource(MinibatchSource&&) = delete;
  MinibatchSource& operator=(const MinibatchSource&) = delete;
  MinibatchSource& operator=(MinibatchSource&&) = delete;
...
namespace std {
template <>
struct hash<::CNTK::DistributedWorkerDescriptor> {
