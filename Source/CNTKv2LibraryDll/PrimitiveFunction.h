...
namespace std {
template <>
struct hash<CNTK::PrimitiveOpType> {
 ...
 private:
  ...
  static NDShape TimesOpOutputShape(Variable& leftOperand, Variable& rightOperand, size_t outputRank, int inferInputRankToMap, bool inferInputDimensions) {
    ...
    for (size_t i = 0; i < numReductionAxes; ++i) {
      if ((leftOperandShape[outputRank + i] != NDShape::InferredDimension && leftOperandShape[outputRank + i] != NDShape::FreeDimension) &&
          (rightOperandShape[i] != NDShape::InferredDimension && rightOperandShape[i] != NDShape::FreeDimension)) {
